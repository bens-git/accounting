          ...
          <template v-slot:header="{ page, pageCount, prevPage, nextPage }">
            <div>
              <v-row>
                ...
                <v-col cols="12" md="4">
                  <v-date-input dense v-model="transactionStore.dateRange" label="Dates" prepend-icon=""
...
const setupForm = async () => {
  ...
  if (props.type) {
    ...
    await itemStore.fetchItems(localType.value.id, transactionStore.location, transactionStore.radius);
  } else {
...
const bookRental = async () => {
  await itemStore.bookRental(localItem.value.value, transactionStore.dateRange[0], transactionStore.dateRange[transactionStore.dateRange.length - 1]);
...
const minEndDate = computed(() => {
  if (transactionStore.dateRange[0]) {
    return transactionStore.dateRange[0];
  }
  return today;
});

const maxStartDate = computed(() => {
  if (transactionStore.dateRange[transactionStore.dateRange.length - 1]) {
    const endDate = new Date(transactionStore.dateRange[transactionStore.dateRange.length - 1]);
