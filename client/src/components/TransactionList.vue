        ...
        <v-row>
          ...
          <v-col cols="12" md="3">
            <v-btn
              ...
              color="success"
              @click="transactionStore.createTransaction"
            >
              New
            </v-btn>
...
watch(
  () => transactionStore.dateRange[0],
  (newStartDate) => {
    if (
      newStartDate >
      transactionStore.dateRange[transactionStore.dateRange.length - 1]
    ) {
      transactionStore.dateRange[transactionStore.dateRange.length - 1] =
        new Date(newStartDate.getTime() + startOfDayInMillis);
    }
  }
);
...
watch(
  ...
  (newEndDate) => {
    if (newEndDate < transactionStore.dateRange[0]) {
      transactionStore.dateRange[0] = new Date(
