<template>
    <div>
        ...
        <div class="radius-input">
            <br>
            <v-slider label="Radius (km)" show-ticks="always" tick-size="10" v-model="transactionStore.radius" step="10"
...
import { useTransactionStore } from '@/stores/transaction.js'; // Your Pinia store

const transactionStore = useTransactionStore(); // Access the Pinia store
...
onMounted(() => {
    ...
    // Check if transactionStore.location is available
    const initialLocation = transactionStore.location || { lat: defaultLatLng[0], lng: defaultLatLng[1] };
    ...
    const addRadius = () => {
        ...
        circleMarker = L.circle([transactionStore.location.lat, transactionStore.location.lng], {
            radius: transactionStore.radius * 1000,
            color: 'blue',
            fillOpacity: 0.2
        }).addTo(map);
    }
    ...
    map.on('click', async (e) => {
        ...
        transactionStore.setLocation({ lat, lng });
        ...
        try {
            ...
            transactionStore.setAddress('Error fetching address');
        }
    ...
    map.on('geosearch/showlocation', async (event) => {
        ...
        address.value = await reverseGeocode(lat, lng);
        transactionStore.setAddress(address.value);
    ...
    watch(() => transactionStore.location, async (newLocation) => {
        if (newLocation) {
